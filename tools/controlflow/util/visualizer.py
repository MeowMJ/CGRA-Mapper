...
class SimulationDataAnalyzer:
    ...
    def __init__(self, kernel_data, output_dir):
        """
        ...
        self.ESCORT_CONFIGS = ['Baseline', 'Neura-L0', 'Neura-L1', 'Neura-L2', 'Neura']
        self.KERNEL_COLORS = ['#A4A3A4','#B0C4E6','#8DA9DC','#FEEDB9','#002060',
                            '#F3B082','#F7CAAB','#C7FAA8','#FFD865']
        self.ESCORT_COLORS = ['#7F7F7F','#EDEDED','#FFF2CC','#FFD966','#FFC000']
    ...
    def load_execution_data(self, task_case: str, csv_name: str, normalized_baseline: int):
        ...
        """
        file_path = f'{self.OUTPUT_DIR}/simulation_{task_case}_{csv_name}.csv'
    ...
    def process_throughput_data(self, task_cases: List[str]):
        ...
        """
        df = pd.read_csv(f"{self.OUTPUT_DIR}/simulation_1_Baseline.csv")
        file_path = f"{self.OUTPUT_DIR}/simulation_1_Baseline.csv"
        normalized_baseline = df['Overall_Execution'].iloc[0] #case1 的 Baseline 的 overall execution time
    ...
    def genFig9(self, fig_path: str):
        ...
        # Collect data
        for case in cases:
            for group in self.ESCORT_CONFIGS:
        ...
        total_bars = len(cases) * len(self.ESCORT_CONFIGS)
        ...
        num_complete_cases = len(x_positions) // len(self.ESCORT_CONFIGS)
        ...
        for case_idx in range(num_complete_cases):
            ...
            start_idx = case_idx * len(self.ESCORT_CONFIGS)
            end_idx = start_idx + len(self.ESCORT_CONFIGS)
    ...
    def genFig10(self, fig_path: str):
        ...
        bars = ax1.bar(x_positions, bar_data, bar_width,
               color=self.ESCORT_COLORS[:len(bar_data)],
               alpha=0.8,
               edgecolor='black',
               linewidth=0.5)
    ...
    def genFig11(self, fig_path: str):
        ...
        total_bars = (len(cases) * (len(self.ESCORT_CONFIGS) - 1)) + 1
        x_positions = np.arange(total_bars)
        bar_width = 0.6
        # Primary Y-axis - Bar chart
        color_dict = {kernel: color for kernel, color in zip(self.KERNEL_NAMES, self.KERNEL_COLORS)}
