...
#ifdef USE_LOOP_TILING
#include "../../common/vector_defines.h"
#define TILE_SIZE_N 23
#endif
...
#elif defined(USE_LOOP_TILING)

// 8B*(n-2)*6 <= 1KB
void kernel_jacobi_2d_vector_tiling(int tsteps,int n, DATA_TYPE **A,DATA_TYPE **B)
{
    _MMR_f64    xU;
    _MMR_f64    xUtmp;
    _MMR_f64    xUtmp2;
    ...
    for (int j=1; j<=size_x; j=j+gvl)
    {
        ...
        xU = _MM_LOAD_f64(&A[1][j],gvl);
        xUbottom = _MM_LOAD_f64(&A[2][j],gvl);
        for (int ii=1;ii<=size_y; ii+=TILE_SIZE_I){
          for (int i=ii; i<=min(size_y, ii+TILE_SIZE_I); i++)
          {
              // Tile Load
              if(i!=1)
              {
...
// 8B*(n-2)*6 <= 1KB
void kernel_jacobi_2d_vector_tiling_systolic(int tsteps,int n, DATA_TYPE **A,DATA_TYPE **B)
{
    _MMR_f64    xU;
    _MMR_f64    xUtmp;
    _MMR_f64    xUtmp2;
...
#elif USE_LOOP_TILING_SYSTOLIC
    for (t = 0; t < tsteps; t++)
    {
      kernel_jacobi_2d_vector_tiling_systolic(tsteps,n, A,B);
      kernel_jacobi_2d_vector_tiling_systolic(tsteps,n, B,A);
    }
#else
    for (t = 0; t < tsteps; t++)
    {
